...
- compatibility with nested-scope symbols from phase 03b
- compatibility with explicit reference availability state from phase 6
- compatibility with updated risk facts from phase 7
...
- [ ] Treat schemas as the contract source of truth for tool behavior.
- [ ] Preserve structural hierarchy and lexical hierarchy as separate data in tool outputs.
- [ ] Do not flatten unavailable reference data into fake zero counts.
- [ ] Do not assume qualified names are always unique after phase 03b.


Structured output schemas are explicitly supported by MCP tools, and when a server advertises an output schema it must return structured results that conform to it. [web:571][web:750] Strong schema discipline is a core MCP best practice because clients and automated consumers rely on those contracts rather than free-text descriptions. [web:747][web:749]
...
MCP tool contracts benefit from structured outputs and structured error reporting rather than untyped strings, because clients can validate and recover more safely. [web:571][web:751][web:741]
...
- [ ] use strict schemas for every input
- [ ] use strict schemas for every output
- [ ] include explicit enums where value sets are finite
- [ ] include array non-empty constraints where required
- [ ] model optional fields explicitly instead of implying them in text
- [ ] keep outputs stable across repeated calls for the same stored state


Do not rely on descriptions alone to express machine-level constraints. [web:749]
...
### Required post-03b rule


- [ ] `ResolveSymbolInput` must support enough narrowing to recover from qualified-name ambiguity
- [ ] if `file_id` is present, use it as an additional narrowing hint
- [ ] if `file_id` is absent, ambiguity must remain explicit rather than guessed away
...
- [ ] do not collapse structural and lexical relationships into one `parent` field
...
- [ ] if `file_id` is provided, apply it as an additional narrowing filter
...
### Exact ambiguity details


If ambiguous:
- [ ] include candidate symbol IDs
- [ ] include candidate kinds
- [ ] include candidate file IDs
- [ ] include candidate qualified names
...
### Required post-03b behavior


- [ ] context output must include structural and lexical relationships separately
- [ ] context output must include `reference_summary.available`
- [ ] context output must not pretend unrefreshed references are zero
...
- [ ] if references were never refreshed, return `available = false`
- [ ] if references were refreshed and none were found, return `available = true` with zero counts
- [ ] keep those two states distinct
...
- [ ] risk payload must preserve `reference_availability`
- [ ] risk payload must preserve `touches_local_scope_only`
- [ ] risk payload must preserve issue codes exactly as returned by the engine
...
#### `test_resolve_symbol_tool_ambiguous`
- [ ] ambiguous lookup returns `ok = false`
- [ ] error code is `ambiguous_symbol`
- [ ] candidate details are present


#### `test_get_symbol_context_tool`
- [ ] context payload exists
- [ ] expected top-level fields exist
- [ ] structural and lexical fields are both present
...
#### `test_get_symbol_references_unavailable_state`
- [ ] never-refreshed symbols return `available = false`
- [ ] refreshed-zero and unavailable are not collapsed


#### `test_analyze_symbol_risk_tool`
- [ ] structured risk result is returned
- [ ] facts, issues, score, and decision exist
- [ ] reference availability facts are preserved
...
- [ ] published schemas match actual handler behavior
...
- [ ] ambiguity handling is deterministic
- [ ] reference availability state is preserved
...
- [ ] output schemas are published and followed
- [ ] error contracts are strict
- [ ] structural and lexical relationships remain distinct in tool outputs
- [ ] reference availability remains explicit
